...
This document explains the image storage configuration for both development and
...
The `docker-compose.prod.yml` includes a volume mount that maps your local
images directory to the container:
...
- Images are mounted from host to container: `./static/images` →
  `/app/static/images`
...
To switch storage backend, update `IMAGE_CONFIG.STORAGE.TYPE` in
