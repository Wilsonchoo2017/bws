...
from api.workers.transforms import hobbydigi_product_to_dict

if TYPE_CHECKING:
    from api.jobs import Job, JobManager


class HobbydigiWorker:
    scraper_id = "hobbydigi"
    max_concurrency = 1
    ...
    async def run(self, job: Job, mgr: JobManager) -> WorkResult:
        ...
        if not result.success:
            raise RuntimeError(result.error or "HobbyDigi scrape failed")

        items = [hobbydigi_product_to_dict(p) for p in result.products]
