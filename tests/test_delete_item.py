...
class TestDeleteItemBasic:
    ...
    def test_given_existing_item_when_deleted_then_returns_true(self, conn):
        ...
        get_or_create_item(conn, "122222")
        assert delete_item(conn, "122222") is True
    ...
    def test_given_existing_item_when_deleted_then_item_gone(self, conn):
        ...
        get_or_create_item(conn, "122222")
        delete_item(conn, "122222")
        assert item_exists(conn, "122222") is False
...
class TestDeleteItemCascade:
    ...
    def test_given_item_with_prices_when_deleted_then_prices_removed(self, conn):
        ...
        get_or_create_item(conn, "122222")
        record_price(conn, "122222", "shopee", 9900)
        record_price(conn, "122222", "toysrus", 12900)

        delete_item(conn, "122222")
        assert _count_rows(conn, "price_records", "122222") == 0
    ...
    def test_given_item_with_be_snapshot_when_deleted_then_snapshot_removed(self, conn):
        ...
        get_or_create_item(conn, "122222")
        _insert_brickeconomy_snapshot(conn, "122222")

        delete_item(conn, "122222")
        assert _count_rows(conn, "brickeconomy_snapshots", "122222") == 0
    ...
    def test_given_item_with_keepa_snapshot_when_deleted_then_snapshot_removed(self, conn):
        ...
        then the snapshot is also deleted."""
        get_or_create_item(conn, "122222")
        _insert_keepa_snapshot(conn, "122222")

        delete_item(conn, "122222")
        assert _count_rows(conn, "keepa_snapshots", "122222") == 0
    ...
    def test_given_item_with_scrape_task_when_deleted_then_task_removed(self, conn):
        ...
        get_or_create_item(conn, "122222")
        _insert_scrape_task(conn, "122222")

        delete_item(conn, "122222")
        assert _count_rows(conn, "scrape_tasks", "122222") == 0
    ...
    def test_given_item_with_image_asset_when_deleted_then_asset_removed(self, conn):
        ...
        conn.execute(
            ...
            VALUES (nextval('image_assets_id_seq'), 'set', '122222',
        ...
        delete_item(conn, "122222")
        count = conn.execute(
            "SELECT COUNT(*) FROM image_assets WHERE item_id = '122222'"
        ).fetchone()[0]
        assert count == 0
