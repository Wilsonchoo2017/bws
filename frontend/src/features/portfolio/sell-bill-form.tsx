...
export interface SellBillEditData {
  billId: string;
  transactions: Transaction[];
}

interface SellBillFormProps {
  onSuccess?: () => void;
  editData?: SellBillEditData;
  onCancel?: () => void;
}
...
export function SellBillForm({ onSuccess, editData, onCancel }: SellBillFormProps) {
  ...
  const [platform, setPlatform] = useState(
    isEdit ? editData.transactions[0].platform ?? '' : ''
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!data.success) {
        setError(data.error || `Failed to ${isEdit ? 'update' : 'create'} sell bill`);
      } else {
        if (!isEdit) resetForm();
        setOpen(false);
        onSuccess?.();
    ...
    >
      <div className='flex items-center justify-between'>
        <h3 className='text-sm font-semibold'>
          {isEdit ? `Edit Sell Bill (${editData.billId})` : 'New Sell Bill'}
        </h3>
        <Button
          type='button'
          variant='ghost'
        ...
        <div className='rounded bg-muted/50 p-3 space-y-1'>
          <div className='flex items-center justify-between text-xs'>
            ...
            >
              ...
              {adjustmentPct < 0 ? 'fees/deductions' : 'extra'})
            </span>
          </div>
          {effectivePrices && (
      ...
      <Button type='submit' size='sm' disabled={submitting}>
        {submitting ? 'Saving...' : isEdit ? 'Update Sell Bill' : 'Save Sell Bill'}
      </Button>
    </form>
  );
}
